...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'place.create': {
    roles: ['content-moderator', 'admin'],
    key: 'new-place-added',
    tenantPath: ['parent', 'place'],
    userPath: [],
  },
  'comment.create': {
    roles: ['content-moderator', 'admin', 'business-owner'],
    key: 'new-comment-added',
    tenantPath: ['parent', 'comment'],
    userPath: [],
  },
  'place.update': {
    roles: ['business-owner'],
    key: 'place-rating-updated',
    tenantPath: ['parent', 'place'],
    userPath: [],
  },
};

const ownerRoles: string[] = ['common-user'];
const customerRoles: string[] = [];
const tenantRoles: string[] = ['content-moderator', 'admin', 'business-owner', 'common-user'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.parent.findFirst({
