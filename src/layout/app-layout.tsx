...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiUserCheck, FiMapPin } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['common-user', 'content-moderator', 'admin', 'business-owner'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<ParentInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/parents` : null),
    () => getParents({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Parents',
      path: isTenantUser ? `/parents/view/${data?.[0]?.id}` : '/parents',
      entity: 'parent',
      service: AccessServiceEnum.PROJECT,
      icon: FiUsers,
    },
    { name: 'Children', path: '/children', entity: 'child', service: AccessServiceEnum.PROJECT, icon: FiUserCheck },
    {
      name: 'Comments',
      path: '/comments',
      entity: 'comment',
    ...
    { name: 'Places', path: '/places', entity: 'place', service: AccessServiceEnum.PROJECT, icon: FiMapPin },
