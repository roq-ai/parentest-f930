...
import { getPlaces } from 'apiSdk/places';
...
import { CommentListPage as ShowCaseList } from 'pages/comments';

function DashboardPage() {
  const { session } = useSession();

  const placeStat = useSWR<PaginatedInterface<PlaceInterface>>(
    () => `/places`,
    () => getPlaces({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Places',
      result: placeStat?.data?.totalCount,
      isLoading: placeStat.isLoading,
    },
    {
      title: 'Comments',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`comments`} passHref legacyBehavior>
                  <Button
