...
import { getPlaces, deletePlaceById } from 'apiSdk/places';
import { PlaceInterface } from 'interfaces/place';

type ColumnType = ColumnDef<PlaceInterface, unknown>;
...
export function PlaceListPage(props: PlaceListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getPlaces({
        relations: ['parent', 'comment.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PlaceInterface>>(
    () => `/places?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: PlaceInterface) => {
    if (hasAccess('place', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/places/view/${row.id}`);
    }
  };
    ...
    hasAccess('parent', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/parents/view/${record.parent?.id}`}>
              {record.parent?.name}
            </Link>
          ),
        }
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/places/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('place', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/places/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('place', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="place" />
        </Flex>

        {hasAccess('place', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/places/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
