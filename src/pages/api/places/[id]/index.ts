...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getPlaceById() {
    const data = await prisma.place.findFirst(convertQueryToPrismaUtil(req.query, 'place'));
    return res.status(200).json(data);
  }

  async function updatePlaceById() {
    await placeValidationSchema.validate(req.body);
  ...
  async function deletePlaceById() {
